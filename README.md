...
TODO - установка через pi imager, описать что нужно образ Lite и зайти в настройки включить ssh, сразу создать пользователя pi и пароль к нему, и установить часовой пояс
Если мы ставим всё с нуля, то раскатываем на карту памяти образ свежей Raspberry Pi OS (она же в прошлом Raspbian). Подключаемся по SSH в консоль.

Если у нас самые первые модели Raspberry Pi (с 512Мб оперативки), то имеет смысл сделать дополнительную настройку. Для этого запустим конфигуратор.
```bash
sudo raspi-config
```
И в разделе Performance Options - GPU Memory понизим количество памяти, выделяемое на видео (до 16Мб).
