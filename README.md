...
Если мы ставим всё с нуля, раскатываем на карту памяти образ свежей Raspberry Pi OS (она же Raspbian). Подключаемся по SSH в консоль и первым делом запустим настройки.
```bash
sudo raspi-config
```
...
Правим его под свои предпочтения (в будущем нужно сделать нормальный файл настроек, а не разбрасывать по разным местам).  
