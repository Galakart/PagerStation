...
В середине установки будет запрошено создание первого пользователя с доступом к API. Необходимо заполнить все данные, что он попросит, после чего установка будет продолжена. Последующих пользователей можно будет создавать уже через само API.
